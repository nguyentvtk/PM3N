'use client';

import React, { useState, Suspense } from 'react';
import { signIn, useSession } from 'next-auth/react';
import { useRouter, useSearchParams } from 'next/navigation';
import { useEffect } from 'react';
import { Eye, EyeOff, Mail, Lock, Loader2, LogIn } from 'lucide-react';
import Link from 'next/link';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import { Separator } from '@/components/ui/separator';
import { Tabs, TabsList, TabsTrigger, TabsContent } from '@/components/ui/tabs';

// ── Error message map ─────────────────────────────────────────────
const ERROR_MESSAGES: Record<string, string> = {
  AccessDenied:       '🚫 Email chưa được cấp quyền. Liên hệ quản trị viên.',
  EMAIL_NOT_FOUND:    '📧 Email không tồn tại trong hệ thống.',
  WRONG_PASSWORD:     '🔐 Mật khẩu không đúng.',
  USE_GOOGLE_LOGIN:   '🌐 Tài khoản này chỉ đăng nhập được qua Google.',
  CredentialsSignin:  '❌ Email hoặc mật khẩu không đúng.',
  OAuthSignin:        '❌ Lỗi đăng nhập Google. Vui lòng thử lại.',
  Default:            '❌ Đăng nhập thất bại. Vui lòng thử lại.',
};

function getErrorMessage(error: string | null) {
  if (!error) return null;
  return ERROR_MESSAGES[error] ?? `${ERROR_MESSAGES.Default} (${error})`;
}

// ── Logo Component ────────────────────────────────────────────────
function AppLogo() {
  return (
    <div className="mx-auto mb-6 w-14 h-14 rounded-2xl flex items-center justify-center brand-gradient shadow-brand">
      <svg width="28" height="28" fill="none" viewBox="0 0 24 24" stroke="white" strokeWidth={1.8}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
      </svg>
    </div>
  );
}

// ── Google Sign-in Button ─────────────────────────────────────────
function GoogleButton({ loading }: { loading: boolean }) {
  return (
    <Button
      id="btn-google-signin"
      type="button"
      variant="outline"
      className="w-full h-11 gap-3"
      onClick={() => signIn('google', { callbackUrl: '/' })}
      disabled={loading}
    >
      <svg width="18" height="18" viewBox="0 0 24 24">
        <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
        <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
        <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
        <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
      </svg>
      Tiếp tục với Google
    </Button>
  );
}

// ── Credentials Form ──────────────────────────────────────────────
function CredentialsForm({ onError }: { onError: (msg: string | null) => void }) {
  const router = useRouter();
  const [email, setEmail]       = useState('');
  const [password, setPassword] = useState('');
  const [showPw, setShowPw]     = useState(false);
  const [loading, setLoading]   = useState(false);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    onError(null);

    const res = await signIn('credentials', {
      email,
      password,
      redirect: false,
    });

    setLoading(false);

    if (res?.error) {
      onError(getErrorMessage(res.error));
    } else if (res?.ok) {
      router.push('/');
    }
  };

  return (
    <form id="form-credentials" onSubmit={handleSubmit} className="space-y-4">
      {/* Email */}
      <div className="space-y-1.5">
        <Label htmlFor="email">Email</Label>
        <div className="relative">
          <Mail className="absolute left-3 top-1/2 -translate-y-1/2 size-4 text-slate-500 pointer-events-none" />
          <Input
            id="email"
            type="email"
            placeholder="ten.nguoidung@pm3n.com"
            value={email}
            onChange={(e) => setEmail(e.target.value)}
            className="pl-9"
            autoComplete="email"
            required
            disabled={loading}
          />
        </div>
      </div>

      {/* Password */}
      <div className="space-y-1.5">
        <div className="flex items-center justify-between">
          <Label htmlFor="password">Mật khẩu</Label>
          <Link
            href="/quen-mat-khau"
            className="text-xs text-blue-400 hover:text-blue-300 transition-colors"
          >
            Quên mật khẩu?
          </Link>
        </div>
        <div className="relative">
          <Lock className="absolute left-3 top-1/2 -translate-y-1/2 size-4 text-slate-500 pointer-events-none" />
          <Input
            id="password"
            type={showPw ? 'text' : 'password'}
            placeholder="••••••••"
            value={password}
            onChange={(e) => setPassword(e.target.value)}
            className="pl-9 pr-10"
            autoComplete="current-password"
            required
            disabled={loading}
          />
          <button
            type="button"
            onClick={() => setShowPw(!showPw)}
            className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-300 transition-colors"
            tabIndex={-1}
          >
            {showPw ? <EyeOff className="size-4" /> : <Eye className="size-4" />}
          </button>
        </div>
      </div>

      <Button
        type="submit"
        id="btn-credentials-signin"
        className="w-full h-11 mt-2"
        disabled={loading}
      >
        {loading ? (
          <><Loader2 className="size-4 animate-spin" /> Đang đăng nhập...</>
        ) : (
          <><LogIn className="size-4" /> Đăng nhập</>
        )}
      </Button>
    </form>
  );
}

// ── Main Login Content ────────────────────────────────────────────
function LoginContent() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const { status } = useSession();
  const [error, setError] = useState<string | null>(null);

  // Error từ NextAuth query param (e.g. ?error=AccessDenied)
  const queryError = searchParams.get('error');

  useEffect(() => {
    if (status === 'authenticated') router.push('/');
  }, [status, router]);

  useEffect(() => {
    if (queryError) setError(getErrorMessage(queryError));
  }, [queryError]);

  if (status === 'loading') {
    return (
      <div className="flex items-center justify-center p-8">
        <Loader2 className="size-7 animate-spin text-blue-400" />
      </div>
    );
  }

  return (
    <div className="w-full max-w-md animate-fade-in">
      {/* Card chính */}
      <div className="rounded-3xl border border-white/10 bg-[#131d35]/85 backdrop-blur-xl shadow-2xl p-8">
        <AppLogo />

        <div className="text-center mb-7">
          <h1 className="text-2xl font-bold text-slate-100">Công Văn & Ký Số</h1>
          <p className="text-sm text-slate-500 mt-1">PM3N — Hệ thống quản lý hồ sơ nội bộ</p>
        </div>

        {/* Error Alert */}
        {error && (
          <div className="mb-5 flex items-start gap-3 rounded-xl border border-red-500/20 bg-red-500/8 px-4 py-3 text-sm text-red-400">
            <span className="shrink-0 mt-0.5">⚠️</span>
            <span>{error}</span>
          </div>
        )}

        {/* Tabs: Credentials / Google */}
        <Tabs defaultValue="credentials">
          <TabsList className="mb-6">
            <TabsTrigger value="credentials">Email & Mật khẩu</TabsTrigger>
            <TabsTrigger value="google">Google SSO</TabsTrigger>
          </TabsList>

          <TabsContent value="credentials">
            <CredentialsForm onError={setError} />
          </TabsContent>

          <TabsContent value="google">
            <div className="space-y-4">
              <p className="text-sm text-slate-400 text-center">
                Đăng nhập bằng tài khoản Google được cấp phép trong hệ thống.
              </p>
              <GoogleButton loading={false} />
            </div>
          </TabsContent>
        </Tabs>

        {/* Divider nhanh */}
        <div className="mt-5 flex items-center gap-3">
          <Separator />
          <span className="text-xs text-slate-500 whitespace-nowrap">hoặc</span>
          <Separator />
        </div>

        {/* Quick Google below credentials tab */}
        <div className="mt-4">
          <GoogleButton loading={false} />
        </div>

        <p className="mt-5 text-center text-xs text-slate-600">
          Chỉ tài khoản được cấp quyền mới có thể truy cập
        </p>
      </div>

      {/* Footer */}
      <p className="text-center mt-5 text-xs text-slate-700">
        © 2026 PM3N Workspace · All rights reserved
      </p>
    </div>
  );
}

export default function LoginPage() {
  return (
    <Suspense fallback={
      <div className="flex items-center justify-center p-8">
        <Loader2 className="size-7 animate-spin text-blue-400" />
      </div>
    }>
      <LoginContent />
    </Suspense>
  );
}
